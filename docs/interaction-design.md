...
### 2.5 保密通道必须像独立产品，而不是后台一个 Tab

- 领导端单独品牌语气、单独导航、单独状态模型。
- 未导入私钥前，页面只讨论“密文消息”和“本地解密”，不出现任何伪明文占位。
...
- 默认进入页面时选中 `会上公开`。
- 切换路由不清空已输入内容。
- 切到 `直达领导` 时，输入区上方追加保密提示：`该问题不会进入大会议题统计。`
- 切到 `直达领导` 时，同时展示一条加密说明：`内容将在你的浏览器本地加密后再提交。`
...
- `公开讨论`：强调“审核后其他同事可见”
- `会上公开`：强调“会前不公开”
- `直达领导`：强调“仅指定收件人查看，不上会，并在本地加密后上传”
...
- `直达领导` 不展示 `查看公开讨论` 作为主路径，可仅保留弱入口或直接隐藏。
...
领导端应与会务后台明显区隔，采用更克制、低信息密度的布局。

设计要求：

- 单页面或极简导航
- 不出现“问题管理”“议题管理”等会务术语
- 首屏先说明“消息为加密存储，需本地解密查看”

### 7.2 私钥导入

私钥导入是领导端第一关键动作，应放在页面顶部主区域。
...
- 未导入时，消息列表可以看见时间桶和状态，但正文区不可见。
- 导入成功后，仅在当前本地会话生效。
- 导入失败时，只提示“私钥无效或格式不正确”，不显示技术细节。

### 7.3 消息列表与阅读区

推荐使用 `左侧列表 + 右侧阅读区` 的桌面布局。

列表字段：

- 时间桶，例如 `2026-03-23 上午`
- 状态：`新消息 / 已读 / 已归档`
